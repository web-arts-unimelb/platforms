; FoA Drupal base makefile.
api = 2
core = 7.x

;
; Drupal core version.
;
projects[drupal][version] = 7.21

;
; Modules to go into sites/all
;
projects[acl][version] = "7.x-1.0"
projects[acl][subdir] = contrib

projects[addressfield][version] = "7.x-1.0-beta3"
projects[addressfield][subdir] = contrib

projects[admin_menu][version] = "7.x-3.0-rc4"
projects[admin_menu][subdir] = contrib

projects[advanced_help][version] = "7.x-1.0"
projects[advanced_help][subdir] = contrib

projects[auto_nodetitle][version] = "7.x-1.0"
projects[auto_nodetitle][subdir] = contrib

projects[better_formats][version] = "7.x-1.0-beta1"
projects[better_formats][subdir] = contrib

projects[browscap][version] = "7.x-1.3"
projects[browscap][subdir] = contrib

projects[bundle_copy][version] = "7.x-1.1"
projects[bundle_copy][subdir] = contrib

projects[caption_filter][version] = "7.x-1.2"
projects[caption_filter][subdir] = contrib

projects[cck][version] = "7.x-2.x-dev"
projects[cck][subdir] = contrib

projects[cck_blocks][version] = "7.x-1.1"
projects[cck_blocks][subdir] = contrib

projects[conditional_fields][version] = "7.x-3.x-dev"
projects[conditional_fields][subdir] = contrib

projects[content_access][version] = "7.x-1.2-beta1"
projects[content_access][subdir] = contrib

projects[content_access_admin][version] = 
projects[content_access_admin][subdir] = contrib

projects[css_splitter][version] = "7.x-1.x-dev"
projects[css_splitter][subdir] = contrib

projects[ctools][version] = "7.x-1.2"
projects[ctools][subdir] = contrib

projects[customerror][version] = "7.x-1.x-dev"
projects[customerror][subdir] = contrib

projects[custom_formatters][version] = "7.x-2.2"
projects[custom_formatters][subdir] = contrib

projects[date][version] = "7.x-2.6"
projects[date][subdir] = contrib

projects[devel][version] = "7.x-1.3"
projects[devel][subdir] = contrib

projects[diff][version] = "7.x-3.2"
projects[diff][subdir] = contrib

projects[draggableviews][version] = "7.x-2.0"
projects[draggableviews][subdir] = contrib

projects[email][version] = "7.x-1.2"
projects[email][subdir] = contrib

projects[entity][version] = "7.x-1.0"
projects[entity][subdir] = contrib

projects[entityreference][version] = "7.x-1.0"
projects[entityreference][subdir] = contrib

projects[entity_translation][version] = "7.x-1.0-alpha2"
projects[entity_translation][subdir] = contrib

projects[features][version] = "7.x-1.0"
projects[features][subdir] = contrib

projects[feeds][version] = "7.x-2.0-alpha7"
projects[feeds][subdir] = contrib

projects[feeds_tamper][version] = "7.x-1.0-beta4"
projects[feeds_tamper][subdir] = contrib

projects[feeds_xpathparser][version] = "7.x-1.0-beta3"
projects[feeds_xpathparser][subdir] = contrib

projects[field_collection][version] = "7.x-1.0-beta4"
projects[field_collection][subdir] = contrib

projects[field_group][version] = "7.x-1.1"
projects[field_group][subdir] = contrib

projects[field_permissions][version] = "7.x-1.0-beta2"
projects[field_permissions][subdir] = contrib

projects[fieldset_helper][version] = "7.x-2.1"
projects[fieldset_helper][subdir] = contrib

projects[field_validation][version] = "7.x-2.2"
projects[field_validation][subdir] = contrib

projects[filefield_sources][version] = "7.x-1.7"
projects[filefield_sources][subdir] = contrib

projects[geocoder][version] = "7.x-1.2"
projects[geocoder][subdir] = contrib

projects[geofield][version] = "7.x-1.1"
projects[geofield][subdir] = contrib

projects[geophp][version] = "7.x-1.6"
projects[geophp][subdir] = contrib

projects[globalredirect][version] = "7.x-1.5"
projects[globalredirect][subdir] = contrib

projects[google_analytics][version] = "7.x-1.3"
projects[google_analytics][subdir] = contrib

projects[httprl][version] = "7.x-1.10"
projects[httprl][subdir] = contrib

projects[i18n][version] = "7.x-1.7"
projects[i18n][subdir] = contrib

projects[ie6update][version] = "7.x-1.x-dev"
projects[ie6update][subdir] = contrib

projects[imageapi][version] = "7.x-1.x-dev"
projects[imageapi][subdir] = contrib

projects[imagefield_focus][version] = "7.x-1.0"
projects[imagefield_focus][subdir] = contrib

projects[imce][version] = "7.x-1.6"
projects[imce][subdir] = contrib

projects[imce_mkdir][version] = "7.x-1.0"
projects[imce_mkdir][subdir] = contrib

projects[imce_wysiwyg][version] = "7.x-1.x-dev"
projects[imce_wysiwyg][subdir] = contrib

projects[insert][version] = "7.x-1.3"
projects[insert][subdir] = contrib

projects[job_scheduler][version] = "7.x-2.0-alpha3"
projects[job_scheduler][subdir] = contrib

projects[jquerymenu][version] = "7.x-4.0-alpha3"
projects[jquerymenu][subdir] = contrib

projects[jquery_update][version] = "7.x-2.2"
projects[jquery_update][subdir] = contrib

projects[ldap][version] = "7.x-1.0-beta12"
projects[ldap][subdir] = contrib

projects[libraries][version] = "7.x-2.0"
projects[libraries][subdir] = contrib

projects[linkchecker][version] = "7.x-1.1"
projects[linkchecker][subdir] = contrib

projects[linkit][version] = "7.x-2.5"
projects[linkit][subdir] = contrib

projects[media][version] = "7.x-1.2"
projects[media][subdir] = contrib

projects[media_feeds][version] = "7.x-2.0-alpha1"
projects[media_feeds][subdir] = contrib

projects[media_youtube][version] = "7.x-2.0-rc1"
projects[media_youtube][subdir] = contrib

projects[menu_admin_per_menu][version] = "7.x-1.0"
projects[menu_admin_per_menu][subdir] = contrib

projects[menu_block][version] = "7.x-2.3"
projects[menu_block][subdir] = contrib

projects[menu_expanded][version] = "7.x-1.0-beta1"
projects[menu_expanded][subdir] = contrib

projects[menu_import][version] = 7.x-1.4
projects[menu_import][subdir] = contrib

projects[menu_node][version] = "7.x-1.1"
projects[menu_node][subdir] = contrib

projects[menu_node_views][version] = "7.x-1.x-dev"
projects[menu_node_views][subdir] = contrib

projects[module_filter][version] = "7.x-1.7"
projects[module_filter][subdir] = contrib

projects[mollom][version] = "7.x-2.3"
projects[mollom][subdir] = contrib

projects[nice_menus][version] = "7.x-2.0"
projects[nice_menus][subdir] = contrib

projects[nodeblock][version] = "7.x-1.2"
projects[nodeblock][subdir] = contrib

projects[nodequeue][version] = "7.x-2.0-beta1"
projects[nodequeue][subdir] = contrib

projects[options_element][version] = "7.x-1.7"
projects[options_element][subdir] = contrib

projects[panels][version] = "7.x-3.3"
projects[panels][subdir] = contrib

projects[pathauto][version] = "7.x-1.2"
projects[pathauto][subdir] = contrib

projects[pathologic][version] = "7.x-2.10"
projects[pathologic][subdir] = contrib

projects[prepro][version] = "7.x-0.10"
projects[prepro][subdir] = contrib

projects[quicktabs][version] = "7.x-3.4"
projects[quicktabs][subdir] = contrib

projects[radioactivity][version] = "7.x-2.6"
projects[radioactivity][subdir] = contrib

projects[realname][version] = "7.x-1.0"
projects[realname][subdir] = contrib

projects[redirect][version] = "7.x-1.0-rc1"
projects[redirect][subdir] = contrib

projects[revisioning][version] = "7.x-1.4"
projects[revisioning][subdir] = contrib

projects[rules][version] = "7.x-2.2"
projects[rules][subdir] = contrib

projects[sassy][version] = "7.x-2.13"
projects[sassy][subdir] = contrib

projects[securepages][version] = "7.x-1.x-dev"
projects[securepages][subdir] = contrib

projects[security_review][version] = "7.x-1.0"
projects[security_review][subdir] = contrib

projects[select_or_other][version] = "7.x-2.11"
projects[select_or_other][subdir] = contrib

projects[smartcrop][version] = "7.x-1.0-beta2"
projects[smartcrop][subdir] = contrib

projects[stringoverrides][version] = "7.x-1.8"
projects[stringoverrides][subdir] = contrib

projects[strongarm][version] = "7.x-2.0"
projects[strongarm][subdir] = contrib

projects[taxonomy_access][version] = "7.x-1.0-rc1"
projects[taxonomy_access][subdir] = contrib

projects[taxonomy_csv][version] = "7.x-5.10"
projects[taxonomy_csv][subdir] = contrib

projects[themekey][version] = "7.x-2.3"
projects[themekey][subdir] = contrib

projects[title][version] = "7.x-1.0-alpha4"
projects[title][subdir] = contrib

projects[token][version] = "7.x-1.5"
projects[token][subdir] = contrib

projects[transliteration][version] = "7.x-3.1"
projects[transliteration][subdir] = contrib

projects[unimelb_settings][version] = "7.x-1.0-beta1"
projects[unimelb_settings][subdir] = contrib

projects[userprotect][version] = "7.x-1.0"
projects[userprotect][subdir] = contrib

projects[uuid][version] = "7.x-1.0-alpha3"
projects[uuid][subdir] = contrib

projects[variable][version] = "7.x-2.1"
projects[variable][subdir] = contrib

projects[views][version] = "7.x-3.5-dev"
projects[views][subdir] = contrib
projects[views][patch][] = http://drupal.org/files/issues/1191928-views-form-override-url_0.patch

projects[views_bulk_operations][version] = "7.x-3.1"
projects[views_bulk_operations][subdir] = contrib

projects[views_slideshow][version] = "7.x-3.0"
projects[views_slideshow][subdir] = contrib

projects[webform][version] = "7.x-4.0-alpha6"
projects[webform][subdir] = contrib

projects[webform2pdf][version] = "7.x-3.0+3-dev"
projects[webform2pdf][subdir] = contrib

projects[webform_autocomplete][version] = "7.x-1.1"
projects[webform_autocomplete][subdir] = contrib

projects[webform_conditional][version] = "7.x-2.x-dev"
projects[webform_conditional][subdir] = contrib

projects[webform_node_element][version] = "7.x-1.2"
projects[webform_node_element][subdir] = contrib

projects[webform_validation][version] = "7.x-1.1"
projects[webform_validation][subdir] = contrib

projects[workbench_access][version] = "7.x-1.0"
projects[workbench_access][subdir] = contrib

projects[wysiwyg][version] = "7.x-2.2"
projects[wysiwyg][subdir] = contrib

projects[wysiwyg_spellcheck][version] = "7.x-1.1"
projects[wysiwyg_spellcheck][subdir] = contrib

;
; Themes to go into sites/all
;
projects[rubik][type] = "theme"
projects[rubik][version] = "7.x-4.0-beta8"
projects[rubik][subdir] = contrib

projects[tao][type] = "theme"
projects[tao][version] = "7.x-3.0-beta4"
projects[tao][subdir] = contrib
